...
paths = [
  {"path": "/get-value",
  "method": "GET",
  "lambda": "killerfrog-api-demo_get_value",
  "produces": "application/json",
  "file": "api_demo_get_value.py",
  },
  {"path": "/store-value",
  "method": "POST",
  "lambda": "killerfrog-api-demo_store_value",
  "produces": "text/plain", # TOCONSIDER remove
  "file": "api_demo_store_value.py",
  }
]
'''
...
def create_lambda(client, lambda_name, lambda_file, api_source_arn):
    ...
    #zipped_bytes = bytes()
    zipped_bytes = io.BytesIO() #zipped_bytes, "w")
    with zipfile.ZipFile(zipped_bytes, "w") as z:
        z.write(lambda_file)
        #z.writestr("asfsfd", "adfsdfsadf")
        #print(zipped_bytes.read())

    #zipped_bytes.flush()
    #print(zipped_bytes.getvalue())
    #exit(0)
    config = dict(
    ...
    try:
      ...
      time.sleep(1) # avoid botocore.errorfactory.ResourceConflictException: An error occurred (ResourceConflictException) when calling the CreateFunction operation: Function already exist: killerfrog-api-demo_store_value
